...
.PHONY: help install build run clean test coverage coverage-backend ci lint format generate-docs docker-data-dirs docker-data-permissions docker-setup run-docker restart-docker stop-docker docker-rebuild
...
CI_OPENAI_API_KEY ?= your-openai-api-key-here
...
help:
	...
	@echo "  make ci                   - Run local CI gate (lint + backend coverage + frontend tests)"
...
ci: lint
	...
	@echo "📊 Backend coverage uses CI placeholder OPENAI_API_KEY to catch config-dependent failures."
	OPENAI_API_KEY="$(CI_OPENAI_API_KEY)" $(MAKE) coverage-backend
	$(MAKE) test-frontend
	@echo "✅ Local CI gate completed"
