    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "sources": [{"title": "ML Basics", "url": "https://example.com/ml", "score": 0.95}],
                "confidence": 0.92,
                "timestamp": "2024-01-01T12:00:00Z",
            }
        }
...
class ProjectRequest(BaseModel):
    ...
    daily_rag_enabled: Optional[bool] = Field(
        default=None, description="Toggle per-project Daily RAG"
    )

    model_config = ConfigDict(
        json_schema_extra={
            "example": {"project_id": "project_123", "project_name": "My New Project"}
        }
    )
...
class SleepCycleRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"project_id": "default", "force_cleanup": False}}
    )
...
class SleepCycleResponse(BaseResponse):
    ...
    memory_usage_before: Optional[str] = Field(
        default=None, description="Memory usage before cleanup"
    )
    memory_usage_after: Optional[str] = Field(
        default=None, description="Memory usage after cleanup"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "version": "1.0.0",
                "timestamp": "2024-01-01T12:00:00Z",
                "dependencies": {"openai": "connected", "llm": "ready"},
            }
        }
    )
