...
### `invocations.jsonl` -> `end_invocation`
...
### `maintenance.jsonl` -> `maintenance` (`record_maintenance`)
...
- `total_tokens_reported`
- `usage_source`
- `usage_estimate_method` (nullable)
- `usage_is_estimate`
- `duration_ms`
- `invocations_query` (object with at least `job_id`, `purpose`)
...
### B) Invocation Fields (`invocations.jsonl`)

#### `invocation_id`
- Type and units: string
- Exact meaning: Unique invocation identifier within the run.
- Where measured: `start_invocation`, propagated to `end_invocation`.
- How computed: Monotonic sequence (`inv_########`).
- Invariants / tolerance: Must pair exactly one start and one end record.

#### `purpose`
- Type and units: enum-like string
- Exact meaning: Invocation functional purpose (`main`, `router`, `tagger`, `sleep`, etc.).
- Where measured: Invocation start/end boundaries.
- How computed: Taken from invocation state established at start.
- Invariants / tolerance: End value must not override conflicting purpose.

#### `model`
- Type and units: string
- Exact meaning: Model identifier used by invocation.
- Where measured: Invocation start/end boundaries.
- How computed: Set at start; may be backfilled at end if start was unknown.
- Invariants / tolerance: Conflicts between start/end are schema errors.

#### `meta`
- Type and units: object
- Exact meaning: Invocation diagnostics/context metadata.
- Where measured: Invocation start/end boundaries.
- How computed: Copied from caller metadata plus optional diagnostic extras.
- Invariants / tolerance: Must be object when present.

#### `start_ts`
- Type and units: string (UTC ISO timestamp)
- Exact meaning: Invocation start timestamp.
- Where measured: Invocation lifecycle boundary.
- How computed: Captured at `start_invocation`.
- Invariants / tolerance: Required for end record and latency derivation.

#### `end_ts`
- Type and units: string (UTC ISO timestamp)
- Exact meaning: Invocation completion timestamp.
- Where measured: Invocation lifecycle boundary.
- How computed: Captured at `end_invocation`.
- Invariants / tolerance: Should be >= `start_ts`.

#### `first_token_ts`
- Type and units: string (UTC ISO) or null
- Exact meaning: Time first token was yielded for streaming main invocation.
- Where measured: Streaming invocation path.
- How computed: Captured at first yielded chunk.
- Invariants / tolerance: Null for non-streaming; may be null if unavailable in streaming.

#### `prompt_tokens_reported`
- Type and units: int (tokens)
- Exact meaning: Prompt token count for one invocation.
- Where measured: `end_invocation`.
- How computed: Provider usage; else estimate; else zero fallback.
- Invariants / tolerance: Non-negative; contributes to `total_tokens_reported`.

#### `completion_tokens_reported`
- Type and units: int (tokens)
- Exact meaning: Completion token count for one invocation.
- Where measured: `end_invocation`.
- How computed: Provider usage; else estimate; else zero fallback.
- Invariants / tolerance: Non-negative; contributes to `total_tokens_reported`.
...
#### `total_tokens_reported`
- Type and units: int (tokens)
- Exact meaning: Total token count for one invocation.
- Where measured: `end_invocation`.
- How computed: Provider total if present; else sum prompt + completion.
- Invariants / tolerance: Must equal prompt + completion for canonical records.

#### `usage_source`
- Type and units: enum (`provider` | `estimate` | `zero_fallback`)
- Exact meaning: Provenance class for invocation usage values.
- Where measured: `end_invocation`.
- How computed: Derived from provider availability and fallback path used.
- Invariants / tolerance: `provider => usage_is_estimate=false`; else true.

#### `usage_is_estimate`
- Type and units: boolean
- Exact meaning: Whether invocation usage is estimated/fallback.
- Where measured: `end_invocation`.
- How computed: Normalized from `usage_source`.
- Invariants / tolerance: Must be true for `estimate` and `zero_fallback`.

#### `usage_estimate_method`
- Type and units: string or null
- Exact meaning: Estimator identifier when estimate path is used.
- Where measured: `end_invocation`.
- How computed: Copied from estimator metadata.
- Invariants / tolerance: Should be null for provider-only usage.

#### `timing.ttfb_ms`
- Type and units: int (ms) or null
- Exact meaning: Invocation time-to-first-token for streaming calls.
- Where measured: `end_invocation` timing payload.
- How computed: Start-to-first-yield delta.
- Invariants / tolerance: Null for non-streaming; invalid streaming values coerced to null+warn.

#### `timing.ttlt_ms`
- Type and units: int (ms)
- Exact meaning: Invocation time-to-last-token/completion.
- Where measured: `end_invocation` timing payload.
- How computed: Direct timing when provided; else `end_ts - start_ts`.
- Invariants / tolerance: Non-negative; fallback to 0 only when derivation fails.
...
### E) Maintenance Fields (`maintenance.jsonl`)

Note: v1 keeps shared canonical key names (`total_tokens_reported`, `usage_source`, etc.) across invocation and maintenance streams. Stream context disambiguates semantics. Optional reporting aliases such as `maintenance_total_tokens_reported` / `maintenance_usage_source` are not emitted in current runtime.

#### `job_id`
- Type and units: string
- Exact meaning: Stable maintenance job identifier.
- Where measured: Maintenance lifecycle.
- How computed: Assigned by sleep scheduler/worker.
- Invariants / tolerance: Required; used to reconcile sleep invocations.

#### `job_type`
- Type and units: enum string
- Exact meaning: Maintenance job class.
- Where measured: `record_maintenance`.
- How computed: Caller value normalized.
- Invariants / tolerance: v1 scope is `sleep`.

#### `status`
- Type and units: enum (`success` | `partial` | `failed`)
- Exact meaning: Maintenance completion status.
- Where measured: `record_maintenance`.
- How computed: Caller/worker completion result.
- Invariants / tolerance: Must be valid enum.

#### `prompt_tokens_reported` (maintenance)
- Type and units: int (tokens)
- Exact meaning: Prompt token subtotal for maintenance job usage rollup.
- Where measured: `record_maintenance`.
- How computed: Aggregated from maintenance-scoped invocations (or provided by worker meta override).
- Invariants / tolerance: Non-negative; contributes to maintenance `total_tokens_reported`.

#### `completion_tokens_reported` (maintenance)
- Type and units: int (tokens)
- Exact meaning: Completion token subtotal for maintenance job usage rollup.
- Where measured: `record_maintenance`.
- How computed: Aggregated from maintenance-scoped invocations (or provided by worker meta override).
- Invariants / tolerance: Non-negative; contributes to maintenance `total_tokens_reported`.

#### `total_tokens_reported` (maintenance)
- Type and units: int (tokens)
- Exact meaning: Total maintenance token usage for the job record.
- Where measured: `record_maintenance`.
- How computed: Aggregated from maintenance-scoped invocations; validated against prompt + completion.
- Invariants / tolerance: Must equal `prompt_tokens_reported + completion_tokens_reported`.

#### `usage_source` (maintenance)
- Type and units: enum (`provider` | `estimate` | `zero_fallback`)
- Exact meaning: Provenance class for maintenance token totals.
- Where measured: `record_maintenance`.
- How computed: Derived from aggregated provider/estimate/zero-fallback subtotals.
- Invariants / tolerance: `provider` only when all contributing usage is provider-reported; otherwise non-provider source is used.

#### `usage_estimate_method` (maintenance)
- Type and units: string or null
- Exact meaning: Estimator method identifier for maintenance totals when estimates are used.
- Where measured: `record_maintenance`.
- How computed: Worker/meta provided estimate method context.
- Invariants / tolerance: Should be null for provider-only maintenance usage.

#### `usage_is_estimate` (maintenance)
- Type and units: boolean
- Exact meaning: Whether maintenance totals include estimated or zero-fallback usage.
- Where measured: `record_maintenance`.
- How computed: Derived from maintenance `usage_source`.
- Invariants / tolerance: Must be true whenever source is estimate or zero_fallback.

#### `after_turn`
- Type and units: int or null
- Exact meaning: Turn index after which maintenance was triggered.
- Where measured: `record_maintenance`.
- How computed: Trigger metadata.
- Invariants / tolerance: Scheduler-triggered runs should use null.

#### `duration_ms`
- Type and units: int (ms)
- Exact meaning: Wall-clock maintenance job duration.
- Where measured: `record_maintenance`.
- How computed: `end_ts - start_ts`.
- Invariants / tolerance: Non-negative; should match timestamp ordering.

#### `invocations_query`
- Type and units: object
- Exact meaning: Reconciliation pointer defining contributing invocations.
- Where measured: `record_maintenance`.
- How computed: At minimum includes `job_id`, `purpose="sleep"`; may include `run_id`.
- Invariants / tolerance: Must identify same invocation set used in maintenance totals.

#### `provider_tokens_total`
- Type and units: int (tokens)
- Exact meaning: Maintenance token subtotal from provider-reported invocations.
- Where measured: `record_maintenance`.
- How computed: Aggregated from matching sleep invocations.
- Invariants / tolerance: Non-negative.

#### `estimated_tokens_total`
- Type and units: int (tokens)
- Exact meaning: Maintenance token subtotal from estimate-derived invocations.
- Where measured: `record_maintenance`.
- How computed: Aggregated from matching sleep invocations.
- Invariants / tolerance: Non-negative.
...
#### `zero_fallback_tokens_total`
- Type and units: int (tokens)
- Exact meaning: Maintenance token subtotal from zero-fallback invocations.
- Where measured: `record_maintenance`.
- How computed: Aggregated from matching sleep invocations.
- Invariants / tolerance: Non-negative.
...
#### `invocation_count_sleep`
- Type and units: int (count)
- Exact meaning: Count of matched sleep invocations included in maintenance rollup.
- Where measured: `record_maintenance`.
- How computed: Count of invocations matched by `invocations_query`.
- Invariants / tolerance: Non-negative.

#### `items_in`
- Type and units: int (count)
- Exact meaning: Number of source items considered by maintenance pipeline.
- Where measured: `record_maintenance`.
- How computed: Worker-reported diagnostic count.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `items_out`
- Type and units: int (count)
- Exact meaning: Number of output items produced by maintenance pipeline.
- Where measured: `record_maintenance`.
- How computed: Worker-reported diagnostic count.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.projects_processed_count`
- Type and units: int (count)
- Exact meaning: Number of projects processed by maintenance run.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.projects_failed_count`
- Type and units: int (count)
- Exact meaning: Number of projects that failed during maintenance run.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.updated_projects`
- Type and units: int (count)
- Exact meaning: Number of projects updated by maintenance outputs.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.
...
#### `meta.pruned_count`
- Type and units: int (count)
- Exact meaning: Number of prune operations/items completed.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.formatted_count`
- Type and units: int (count)
- Exact meaning: Number of formatting operations/items completed.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.skipped_no_daily`
- Type and units: int (count)
- Exact meaning: Number of projects/items skipped due to missing daily data.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional diagnostic; non-negative.

#### `meta.errors`
- Type and units: array
- Exact meaning: Diagnostic list of maintenance errors encountered.
- Where measured: `record_maintenance`.
- How computed: Worker diagnostics.
- Invariants / tolerance: Optional; does not change canonical totals fields directly.
